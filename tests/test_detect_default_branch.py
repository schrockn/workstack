...
def test_detect_default_branch_uses_remote_head_master() -> None:
    ...
    git_ops = GitOpsBuilder().with_default_branch(repo_root, "master").build()
...
def test_detect_default_branch_uses_remote_head_main() -> None:
    ...
    git_ops = GitOpsBuilder().with_default_branch(repo_root, "main").build()
